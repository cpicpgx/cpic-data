  ...
  public void export() throws Exception {
    try (Connection conn = ConnectionFactory.newConnection();
         PreparedStatement geneStmt = conn.prepareStatement(
            """
            select distinct d.geneSymbol, g.lookupmethod
            from diplotype d join gene g on (d.genesymbol=g.symbol)
            where g.symbol != 'CFTR' order by 1
            """
         );
         PreparedStatement dipStmt = conn.prepareStatement(
            """
            select d.diplotype, d.generesult, d.ehrpriority, d.totalactivityscore
            from diplotype d
            where d.genesymbol=? order by d.diplotype
            """
         );
